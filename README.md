...
| [awesome-content-graphics](skills/awesome-content-graphics) | Makes the one image a post ships with, offline: a user-sized set (10, 20, 30 or any number) of self-contained HTML/CSS graphics rendered locally across 12 visual archetypes, built from the supplied facts and the user's own look inputs, gated on the canvas language and then the headline. No image service, no API key |
