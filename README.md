...
| [awesome-content-repurpose](skills/awesome-content-repurpose) | Turns one existing text (a link, a file, pasted notes) into platform-native posts: a source-module map that keeps a guide a guide, an interview for language, idea, voice and emoji, verified facts with locked comparison peers, campaign anchors reused verbatim under per-platform frames, a counted audit |
| [awesome-content-graphics](skills/awesome-content-graphics) | Draws the image a post ships with and picks one with the user: the agent composes every picture itself from a reference read for what the picture is about, the user names the count (10, 20, 50, 100), the set opens as a folder to answer by number, another round rebuilds it from different ideas, and the pick is saved into a folder of its own. No image service, no key, nothing uploaded, and no template either |
| [awesome-content-image-adapter](skills/awesome-content-image-adapter) | Adapts one finished image into the frame every platform shows it in, one PNG per platform slug: the list and each surface's frame shape read from the canonical platform table, so nothing is restated. Standalone it writes a session folder and opens it; in a chain it runs straight after the graphics pick and drops the files beside the post they belong to |
