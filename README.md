...
| [awesome-content-graphics](skills/awesome-content-graphics) | Makes the one image a post ships with, offline: a user-sized set (10, 20, 30 or any number) of self-contained HTML/CSS graphics rendered locally and spread across twelve visual archetypes — statement, proof object, split compare, chain, poster, catalog grid, transcript, cut paper, emphasis diagram — built from the supplied facts and the user's own look inputs (brand palette, reference images, an approved render), gated on the canvas language and then the headline before anything is drawn. Called on its own it hands over the whole set, opens the folder and offers another batch on top; called by a post skill it ends on a pick gate |
