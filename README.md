...
| [awesome-content-graphics](skills/awesome-content-graphics) | Makes the one image a post ships with, offline: a user-sized set (10, 20, 30 or any number) of self-contained HTML/CSS graphics rendered locally and spread across twelve visual archetypes — statement, proof object, split compare, chain, poster, catalog grid, transcript, cut paper, emphasis diagram — built from the caller's facts and the user's own look inputs (brand palette, reference images, an approved render), with two gates: the headline as text first, the picture from a gallery second |
