...
- One variant of English per text — a document half in one variant and half in another was written by more than one hand, or by one that stopped paying attention, and for machine-written text the drift between sections is a tell of its own. American spelling is the default and holds unless the user named a variant, supplied a style guide or voice profile fixing one, or handed over a source this work has to match. `references/spelling-variants.md` carries the families worth checking, the words that follow none of them, why `-ize` proves nothing on its own, and what never counts: quotations, proper nouns, identifiers and code.
...
| `references/llm-fingerprints.md` | Model fingerprints by vendor, in two tiers — community-observed tells and vendor-documented prose defaults: OpenAI GPT-5.5 / 5.6, Anthropic Claude Fable 5.1 / Fable 5 / Opus 5.5 / Opus 5 / Sonnet 5 / Opus 4.8, Google Gemini 3.x (+ Deep Research), xAI Grok 4.3, DeepSeek V4, Qwen 3.7, Meta Muse Spark, Mistral Large 3 / Magistral, Perplexity, Amazon Nova, Cohere Command A+ | When the author or executor model is known (Model identity, Working rules); when working with fresh 2025–2026 text |

Where the marker sweep is easier written as a throwaway script than as a grep, the interpreter's name is platform-dependent: `python3` on Linux and macOS, `py -3` or `python` on Windows, where a bare `python3` often resolves to nothing or to a store stub. Check which one answers (`python3 --version`, then `py -3 --version`) and use that, rather than assuming either. Nothing the sweep needs lives outside the standard library.
