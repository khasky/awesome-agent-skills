...
4. A website — call the Skill tool with "awesome-style-mimic" for its Learn mode and treat the guide it returns as one more evidence source. A crawled site is a *brand* voice: when it is the user's own site it informs the profile, and when it is someone else's it may only ever be a labeled reference, never the profile itself.
