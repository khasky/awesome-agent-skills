...
- Filename parses as `YYYY-mm-dd_HH-mm_<pub-timezone>_<title>_<platform>.<ext>` — exactly 5 `_`-separated fields, platform one of the canonical slugs: `facebook-wall` · `facebook-group` · `linkedin` · `reddit` · `tumblr` · `mastodon` · `bluesky` · `x` · `threads` · `wonderful-dev` · `hackernoon` · `devto` · `patreon` · `ko-fi` · `buymeacoffee` · `instagram` · `tiktok` · `bastyon` · `pinterest` · `vk` · `telegram`. This list is shared with `awesome-content-campaign`; the two are edited together or a campaign produces files this skill rejects.
