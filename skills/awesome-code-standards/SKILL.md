...
- No value the code declares — A comment names what a value is for, never what it is: not the lengths of a border-radius shorthand, a quoted `max-height`, a colour, a timeout, a bare number mirroring a constant, and not a selector, class name or storage key the source holds. The comment is the one copy no rename sweep reaches. A value declared nowhere in the repo (a vendor limit, a protocol's fixed width) may be named, with what fixes it.
