...
- Property and fuzz tests — anything parsing untrusted bytes (file formats, protocol frames, user markup) gets property-based or fuzz coverage with the failing corpus committed as regression seeds; example-based tests alone systematically miss the inputs attackers try. A standing hostile-input fixture set (`../../etc/passwd`, `' OR 1=1--`, `<img onerror=…>`, 10MB strings, RTL/zero-width Unicode) runs against every input-handling seam. Harness rules (determinism, size validation, resetting global state), per-ecosystem fuzzers (libFuzzer/AFL++/cargo-fuzz/Atheris/Go), sanitizer builds (ASan/UBSan), coverage-as-signal, and `wycheproof` crypto vectors are in [references/fuzzing.md](references/fuzzing.md). Read it when the seam under test parses untrusted bytes, and not for ordinary unit work.
- The red → green loop — one seam, one test, one minimal implementation per cycle, in vertical slices: write the failing test, watch it fail, write only enough code to pass it, repeat. Writing every test first and then all the implementation tests *imagined* behavior — the tests pin the shape you guessed at rather than the behavior that turned out to matter, and they go insensitive to the changes that count. Refactoring is not a third step of this loop: reshaping the code belongs to the review stage (`awesome-code-review`, or `awesome-code-cleanup` for a repo-wide pass), where the tests written here are what makes it safe.
