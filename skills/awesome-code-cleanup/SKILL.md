...
description: 'Repo-wide audit and cleanup of AI-like code noise — comments to a strict best-practice bar (delete redundant/narrating, condense bloated, fix stale or misleading, keep only load-bearing knowledge), plus a read-only audit mode, an opt-in refactor mode for vague names, dead/duplicated/speculative code, and over-abstraction, and a detection mode that hunts dead code and logical inconsistencies with mechanical sweeps and execution-verified evidence — cleanup passes strictly behavior-preserving and verified. Use whenever the user asks to clean up / prune / audit / condense comments, remove "obvious" or AI-generated comments, make code self-documenting or human-readable, de-slop vibe-coded files (code, not prose), apply a "comments only when necessary" or YAGNI/KISS pass, or says things like "чистка комментариев", "убери лишние комментарии", "comment hygiene", "de-comment", "почисти код". Also use when a code review found comment bloat or naming noise and the user wants it fixed repo-wide rather than file by file, and when the user asks to find dead or unused code, orphan modules, unused exports, unused i18n keys or env vars, or logical inconsistencies between code, comments, docs and CI — "find dead code", "найди мёртвый код и несоответствия", "неиспользуемый код", "логические несоответствия" — via the detection mode (findings first, fixes opt-in per finding). Do not use to de-slop prose, Markdown, or documentation text — use awesome-humanize-en or awesome-document-style for that; for a whole-repo detection sweep of machine-written markers across code, tests, docs and CI (catalog, verdict, verified non-findings) use awesome-slop-audit.'
license: MIT
metadata:
  author: Khasky
