...
description: "Behavior-preserving cleanup of AI-like code noise: narrating comments, dead code, vague names, over-abstraction; applies a slop-audit report. Use when asked to clean up code or comments, or to de-slop."
license: MIT
metadata:
  author: Khasky
...
- Detection (on request) — when the user asks to find dead/unused code or logical inconsistencies ("find dead code", "unused code", "logical inconsistencies"): the DETECTION MODE pipeline — mechanical sweeps, evidence-contract subagents, execute-to-verify. Findings first; fixes only for the findings the user picks.
...
- Hold one variant of English across the comments of a repository. `colour` in one file and `color` in the next is two generators, not two authors, and the repository reads as assembled rather than written. American spelling is the default unless the repository's own prose is consistently in another or its rules name one, and then that one wins everywhere. Fix the minority side only: an identifier keeps its spelling whatever the comment says, because renaming it breaks what it names. `awesome-humanize-en/references/spelling-variants.md` carries the families to check and the `-ize` trap; without that skill, the default named here is the whole rule.
- Normalize AI-slop punctuation in comments — a developer typing a comment reaches for keyboard characters, not typographic glyphs. In comment text, replace the em/en-dash `—`/`–` with a plain `-` (or reword), the ellipsis `…` with `...`, curly quotes `" " ' '` with straight `"` `'`, and decorative bullet/arrow glyphs (`▸ ► ▪ ● ◦ ‣ → ⇒`) with `-` or plain words; strip stray emoji (`✅ ❌ 🚀`) and non-breaking/zero-width spaces. The test is the keyboard, not a list of glyphs: what a developer can type without an alt-code, a compose key or a paste is ``!@#$%^&*()_+-=`~|}{[]\":;'>?/.,<``, the letters and the digits, and nothing else belongs in comment text. Superscript digits (`²`, `³`), fractions (`½`), `°`, primes, `≤`, `×` and their neighbours all fail it — write `^2`, `1/2`, `deg`, `<=`, `x`. These are machine-generation tells — a comment carrying several is itself a signal the block was AI-written and worth a closer read. Edit only the comment text; never touch these characters inside string literals, regexes, identifiers, or data, and leave a glyph that is genuinely load-bearing (a comment documenting that exact character, or intentional non-ASCII in a non-English codebase's own convention).
- Take every value the code declares back out of the comment text. A comment quoting the literals of a rule it sits beside — the four lengths of a border-radius shorthand written out to show which corners round, a width floor copied as `max(18em, ...)`, a `max-height: 28em`, a colour, a timeout, a retry count — is wrong from the next tweak of that declaration onward, and nothing in the build notices. Say what the value is for and let the reader read it at its source: the corner the shorthand rounds, the width floor the stylesheet sets, the ceiling the picker gets. Units are not the test — a bare number mirroring a constant fails the same way. A value that is declared nowhere in the repo (a protocol's fixed field width, a vendor limit, a spec's cap) is the exception and stays, with what fixes it named.
- The same for selectors, class names, file-local keys and any other identifier the code holds: `// Must equal the track count of the .khasky-emojery-grid rows` and `// the ring animation lives in picker.css` both ask the reader to keep two files in sync by hand, and the next rename greps the stylesheet and the source but not the prose. Name the thing in words — the grid's track count, the ring animation — and drop the spelling. Where the coupling is real, the fix is in the code: a shared constant, a generated name, a test that fails when the two drift. Report that in a default comment pass; do it in refactor mode.
- Rewrite a comment that opens by locating itself. `// Validated here rather than where it is consumed: the override feeds both the compiled bundle and the manifest...` spends its first clause telling the reader where they already are and its second arguing against a placement nobody proposed. Cut the frame and open on what holds: `// The override feeds both the compiled bundle and the manifest, and a value only one of them parses puts them on different origins with the build still green.` Same for "Done here because", "Handled at this level rather than", "Kept in this module so that" — the position of the comment is the code's answer, not the comment's.
- Keep a list inside a comment flush with the comment's own text: one space after the marker, wrapped lines starting where the marker starts, no column alignment and no indented run of `//   ` to hang the items under an intro. The padding is a model laying out a rendered answer; a developer typing `//` on each line gets no hanging indent for free and does not add one. Bullets and numbered steps alike.
- Rewrite a comment that opens with a short label, a colon, and then its own explanation. `// Stable across retries either way: the stored key for rows written since enqueue started stamping one, the id+ts derivation for older rows.` The head announces the topic and the tail explains the head, which is how a model organizes an answer; a developer writes the sentence that already carries the fact — `// Both paths keep the key identical across retries - the stored one for rows written since enqueue began stamping it, id+ts for older rows.` A colon joining two different things stays: a name and its value (`// Firefox: no support before 121`), a list, a URL, a ratio, a clock time.
- Strip English inflection off identifiers in comment text. `// The same backoff signInThroughAuthPage's loop takes.` glues an `s` onto a symbol that has to stay greppable, and the possessive reads as prose about a character in a story. Recast so the name stands bare: `// The same backoff the loop in signInThroughAuthPage takes.` The same for plurals and every other suffix — `the useStates` becomes "the useState calls", `two config.jsons` becomes "both config.json files". A symbol whose own spelling ends in `s` keeps it.
- A platform, service or product name standing in front of the thing it owns takes no `'s` either: `X's data-testid` and `GitHub's star-form action` become `X data-testid` and `GitHub star-form action`. The possessive is the register of prose about people, and the bare pair is what the reader greps. This is the name-plus-identifier position only — a name used as an ordinary subject keeps its possessive (`each is that site's to define`).
- Replace borrowed diction in comment text with the plain wording it displaces — the ceremonial verb (`mint`, `honored`), the metaphor standing in for a mechanism (`beat`, `slack`), and the road-not-taken contrast (`rather than`, and the mirrored `X, not Y` it becomes when the words are swapped). The entries, their replacements, the sweep command and the look-alikes live under Borrowed diction in `awesome-humanize-en/references/lexicon.md` (read it when that skill is installed; `awesome-slop-audit` reports the category against the same file). Rewrite only where the hit is comment text — inside a string literal, an identifier, a test title or product data the word is content and stays.

### REFACTOR MODE ONLY (skip in a default comment pass)

- Rename vague identifiers (`data`, `result`, `item`, `temp`, `helper`, `manager`, `processor`, `handler`, `utils`, `processData`, `handleThing`) to the project's own domain names — taken from its glossary (`CONTEXT.md`, a domain doc) or the vocabulary its code and tests already use, never from a synonym you coined — local and non-exported only; renaming public exports or serialized fields needs explicit approval. Source new names from the domain terms already used in nearby code, tests, database fields, API docs, and UI labels. Before applying, produce a rename table — old → new, scope, reason, public-API risk — and apply only the low-risk rows.
...
- Name the shape in the shared vocabulary: module, interface, depth, seam, adapter, leverage, locality, defined once in `awesome-architecture-audit/references/design-vocabulary.md` (read it when that skill is installed). Using those words rather than "component", "service" or "boundary" is what lets a cleanup report and an architecture audit describe the same module without a translation step.
...
Done when: every in-scope file has been read in full, every comment carries one of the four verdicts, and each partition's non-comment diff is empty or maps line-for-line to a declared rename.
...
Done when: lint, every typecheck config, the test suites and the formatter check have each been run and their exit codes read, and every failure is attributed to the pass or shown to pre-date it.
