...
One render per shape serves every platform that names it. The files are still written one per slug, because a post and its picture sharing a name is what makes the folder readable.
...
- The file count equals the number of table rows whose `Image` column names a shape.
