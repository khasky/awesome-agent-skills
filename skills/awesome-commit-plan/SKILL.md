...
description: "Turns a codebase into a commit plan: a deep read of the source, a navigation map of its modules and their dependency direction, then a split into commits where every one builds and tests on its own, so the series is bisectable end to end. Each commit is verified by replaying the ladder in a scratch clone and running the repository's own gates, never asserted. Messages follow a strict subject and body ruleset that keeps machine-written prose out of the log. The only output is a plan file, numbered #1 to #N, with no dates and no preamble. Use when asked to 'split this project into commits', 'plan the commit history for this repo', 'how should I break this into commits', 'prepare a bisectable commit series', or in Russian 'разбей проект на коммиты', 'составь план коммитов', 'как разбить это на коммиты'. Takes a repository URL or a local path: '/awesome-commit-plan <url-or-path>'. Do not use to execute the plan against git history — that is awesome-git-history-rebuild; not to squash an existing history — awesome-git-history-reset; not to review a diff — awesome-code-review."
license: MIT
metadata:
  author: Khasky
