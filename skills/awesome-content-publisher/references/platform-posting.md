...
**Patreon does not autolink, and its link popover cannot be driven.** Every `https://…` in the body publishes as plain text. The repair path exists in the UI — select the URL's line (`Home`, `Shift+End`, assert `window.getSelection()`), click the toolbar's `button[aria-label="Link"]`, fill `input[placeholder="Type or paste URL"]`, press `Add` — and the selection and the toolbar both work, but the popover's input refused `focus()` across three attempts, so nothing could be typed into it. **Budget one attempt, then record the dead URLs as a defect and tell the user**; the post is otherwise fine and the fix is seconds by hand. Leaving the editor afterwards raises a `beforeunload` — accept it to discard the half-applied change, since the published post is already correct.
...
**Submit needs `el.click()` from `page.evaluate`, and the button to take is the first `Post`.** Coordinate clicks and `handle.click()` both fail here (the latter on actionability), while the untrusted click publishes immediately; the textarea clearing to length 0 is the confirmation. Two buttons read `Post` — take `btns[0]`; the second is disabled.
