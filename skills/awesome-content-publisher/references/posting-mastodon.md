...
A trailing hashtag line does not appear in `.status__content` on the published post — Mastodon lifts it into a `.hashtag-bar` below the body, with each tag a live `/tags/<name>` link. A tail comparison against the source therefore fails on a perfectly intact post. Check the tag bar separately and treat the move as the platform's own rendering, not a loss.
