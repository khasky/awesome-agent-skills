...
A publishing run is a chain of round trips, and every round trip costs a turn. One run took 213 minutes over 40 platforms, about five and a half minutes each, and almost none of that was the platforms being slow. The rules below are what the same work costs when the run stops paying for its own habits.

**Reach for the plain tools first, and script only what they cannot express.** `browser_navigate`, `browser_find` plus `browser_click` on the ref it returns, `browser_fill_form`, `browser_select_option` and `browser_file_upload` finish the whole job on a composer built from ordinary form controls - a subject input, a body textarea, a submit. `mataroa` and `dreamwidth` are exactly that, `blogger`'s settings sidebar is, and on those a generated script is pure overhead. `browser_run_code_unsafe` earns its place only where the composer needs something the tools cannot produce: a `ClipboardEvent` carrying `text/html` into a rich editor, a Range built over a block before a toolbar acts on it, a `DataTransfer` drop, a seeded `.ql-clipboard`, a dispatched `change` after `setInputFiles`, or a read that *is* the evidence (natural dimensions, block and heading counts, a selection's length). Decide per platform from its `posting-<slug>.md`, not by habit. A run that scripted everything spent its time writing code for pages that had a `Save` button.

**Never write the script to disk unless the payload demands it.** The `filename` route exists for scripts carrying a body's HTML or a base64 image; everything else goes inline in `code` and saves the write that would precede it. Where files are genuinely needed, generate every platform's payload in one pass at the start of the run - one generator, one output folder, all bodies - rather than running a generator per platform in the middle of each composer.

**Three calls per platform, four where an upload has to be proved.** Probe, fill and read-back belong in a single call that returns the whole evidence object; the submit is its own call because it is the gate; the published-page audit is the third. A run that made a separate call for every field, every selector hunt and every poll spent most of its time waiting for its own turns rather than for the page.

**No padded pauses.** 500 ms after a field is filled, 500 ms after an attachment lands, then act. Nothing else gets a fixed sleep. Everything genuinely asynchronous - an upload finishing, a panel mounting, a navigation, a draft id appearing in the URL - is waited for by its own condition with `page.waitForFunction(() => <the thing you are about to read>, { timeout })`, which returns the moment it is true and costs nothing when the answer is already there. A run that slept three to twelve seconds after every action and polled uploads on a four-second loop spent over an hour asleep. The pacing that protects an account is between *posts*, not between keystrokes: same-platform spacing in `SKILL.md` Phase 7 is unaffected by any of this.

**Do not re-derive what `posting-<slug>.md` already records.** Go straight to the selector the file names and enumerate the DOM only when that read returns null. A run reading a stale installed copy of this skill rediscovered, slowly, findings that were already written down - which is the argument for keeping the installed copy current before a run rather than after it.

**A control that does nothing: work this list before touching the click ladder.** Six causes accounted for every dead control across one run, and each is one read away from being ruled out.

1. **You clicked a wrapper, not the control.** `div.actions` around HackerNoon's `Submit Story for Review!` carries the same `innerText` and is 485 px wide, so a query sorting candidates by width picks it and clicks nothing. Filter to `BUTTON` with a real width *and* height.
2. **The rect is outside the viewport.** LiveJournal's `Tune in and publish` reported a `y` of about -1100 while sitting visibly at the foot of the page; Hashnode's in-dialog `Publish` sat at `y` 2100 on a 1080-tall viewport. `scrollIntoView({ block: 'center' })`, re-read the rect, then click - or fall back to `el.click()` when the rect stays off-screen.
3. **The rect belongs to a hidden input rather than the visible row.** Substack's delivery checkbox reports a real 20x20 box about 250 px below the row the reader sees, and `elementFromPoint` there returns the panel container. Take the rect of the leaf element whose text is the label and click its centre.
4. **There is a 0x0 decoy of the same name.** HackerNoon ships a `div.modal` reading `Confirm?YesNo` from page load; Blogger's `CONFIRM` and Hashnode's header `Publish` each have a zero-sized twin. Filter by width before choosing.
5. **The menu is a portal outside `[role=menu]`.** Substack's `Insert image` list and Telegraph's block buttons render as plain elements elsewhere in the DOM; a probe scoped to menu roles reports an empty menu and the run concludes the click did nothing. Find the entries as leaf elements matched by exact text.
6. **The platform is refusing, not broken.** Read the validation text beside the submit and take a screenshot before the second click, per the Phase 7 rule.

**When the uploader refuses, the picture can still be the right one.** Several platforms in one run would not take a file: the picker demanded a separate sign-in (`blogger`), the file exceeded a hard limit (`mataroa`, 1 MB), the chooser opened and no upload request ever followed (`telegraph`, `teletype`), or the dropzone stalled (`bastyon`). Where the campaign ships one image per platform and those files are byte-identical - check with a hash, never by eye or by dimensions - an address returned by a platform already published in this run is the same picture, and writing it into the body as a URL beats shipping no image at all. Two conditions: prove the bytes match before reusing an address, and record the hotlink as `degraded` naming the host, because the entry now depends on a service the user did not choose for it. Platforms that only ever take an address (`dreamwidth`) already publish last for this reason; a platform that merely failed to upload joins them by accident, so run the image-hosting platforms early.

**Pick a dialog by what it contains, never by its `aria-label`.** One composer mounts three `[role="dialog"]` nodes at once, and the one whose label names the task is an empty shell: zero editable nodes, zero file inputs, the label's own words as its entire text. The working composer is a sibling identified only by `aria-labelledby`. A run that queried the labelled node read an uninitialised dialog, recorded the platform `failed`, and was wrong twice. Select the dialog that holds a `[contenteditable="true"]` or the input you are about to use, and where nothing is found, dump every dialog's editable and file-input counts before concluding the page is broken.

**The bridge itself can wedge, and a navigation is what clears it.** After one interaction - a `Control+a` inside a particular field - every subsequent call parked for the full 120 seconds, including `browser_tabs` and `browser_handle_dialog`, which is what tells this apart from a slow page: a wedge that swallows a tab listing is not the site being slow. Stopping the parked calls does not help, and neither does waiting: five minutes and three waits changed nothing. `browser_navigate` to another URL cleared it at once. Whatever was in the composer is usually still on the server as a draft, so reopen the draft and check before assuming a refill is needed. Record the interaction that caused it in that platform's notes, because the second occurrence is cheaper to avoid than to recover from.
