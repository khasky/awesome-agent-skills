...
Every file begins with exactly these 6 keys, in this order, and no other:
...
- **Files and frontmatter** - exactly the requested set, canonical names, the 6 keys in order plus `attachments` where Phase 11 wrote one, no forbidden key, `voice` and `creativity` equal to the interview answers and `creativity` the same digit on every file, the same 1-3 link set everywhere, hashtags an ordered subset of the pool.
