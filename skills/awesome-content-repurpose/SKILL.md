...
- Contrast frames as the default shape: `not X, but Y`, `it is less about X than Y`, `the real question is not X`. One per post at most, and never as the opener.
...
The mechanical half runs first and it is a script, not a reading: `node scripts/gate.mjs repurpose/<slug>/posts` (add `--subset` when the user trimmed the platform list). It settles, in one pass and in seconds, everything that has a countable answer: filename shape and platform slug, frontmatter agreeing with the filename, title length and terminal period, attachments present on disk, ASCII punctuation, `#show` and internal labels, assistant domains, a URL in parentheses, calendar dates, the literary and hype word lists, the hard caps at their 98% margin, the depth bands, one-line paragraph runs and their share, the banned opener shapes, emoji-bullet rows, body hashtag counts against each platform's norm, duplicate platforms, the file count, and the share of posts opening on `I` across the run. Exit 0 or one `slug: check: detail` line per finding. A finding is fixed in the file, and the script is run again until it exits 0; no judgment pass starts while it does not, because a reader spending attention on a post that is over its cap is attention spent twice.
...
- Opening on an abstract category doing something a category cannot do (`AI video crossed a line`, `AI code review is moving from X to Y`), on a vague event with a vague time (`Something important happened this week`), or on a tease that withholds the subject to build suspense.
- Opening every post in the run on `I think`. Variety across the seven working shapes is the rule; one friendly shape repeated across the whole set is the same uniformity tell as one corporate shape repeated across it.
...
- Re-verifying 26 platforms' caps and formats on every run when the tables already hold them and the user was never asked whether they wanted the check.
