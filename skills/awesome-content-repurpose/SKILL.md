...
- `references/platform-specs.md` — the 26-file output contract, the hard character caps, the depth band each platform's version is written to, and the validation pass that checks all of it.
...
Then check the claims that will not survive being wrong. A source is the boundary on what a post may say; it is not evidence that what it says is still true. Anything current, version-sensitive, priced, benchmarked, install-related, support-matrix-related or counted (stars, downloads, rank) is verified before it fans out, in this order: the official site, the official repository, the official documentation, the paper or model card, the vendor's own announcement. Reputable secondary sources only when none of those carries it.
...
Numbers that came from the project rather than from an independent test say so: `the project reports`, `the paper reports`, `the README shows`. A project benchmark presented as a universal guarantee is a fabrication with a citation attached. Fast-moving counts are date-stamped in the notes and written evergreen in the post (`200k+`, `when it landed`), because a star count is stale within the week.

An install command is verified or it is not published. Never invent one, and prefer the official quick-start path over a source build unless the source is specifically about building from source.
...
The caps in `references/platform-specs.md` are the floor this phase works from, and live research may only lower them. A number that cannot be verified does not become "unknown", it stays at the value in that table; a number verified lower replaces it; a number that looks higher is treated as unverified and the table wins. Two of those caps are invisible in the composer — `peerlist` truncates silently and `wonderful-dev` refuses server-side with nothing on screen — so a run that trusts what the UI shows will ship both broken.
...
9. Shape — the countable half of `references/authored-style.md`, run with a script rather than by reading. Per post: no run of three or more consecutive one-line paragraphs; in anything over 1500 characters the one-line share under 45%; the opener matching one of the seven working shapes and none of the five banned ones; and across the run, at most about half the posts opening on `I` and no single shape covering the whole set. A run finding is a rewrite of the paragraph, restoring the clauses that were cut out of it — never a reflow of its neighbours to match.
10. Depth — every post inside its band in `references/platform-specs.md`. Under the band means the post was not written, and the two that fail this most often are `ko-fi` and `buymeacoffee`, whose composers look like feed boxes and whose surfaces are blogs. Over the band with nothing added means padding. Either way the fix is a rewrite at the right length, never a trim or an inflation of the existing text.
11. Length — counted, not eyeballed, against the Phase 3 cap for each platform, hashtags included, and counted the way that platform counts: a URL that costs a fixed 23 characters (`x`), CJK and emoji that cost two, a title that shares the body's budget. The caps in `references/platform-specs.md` are the ceiling; Phase 3 research may only have lowered them. Every post is checked at 98% of its cap, not at 100%, because two of these truncate or refuse with nothing on screen: `peerlist` accepts an over-length body and publishes it cut mid-URL, and `wonderful-dev` refuses server-side with the error only in the page console. A post that ends on its link is the worst shape for an unverified cap: put the link where losing the tail would not destroy the post, or shorten until the margin is real. An over-cap post is rewritten to the band, never trimmed by dropping its tail.
12. Media — every declared attachment exists, matches the platform's verified formats, has alt text describing what the image says. Where the image was generated rather than supplied, the attachment is the variant the user chose at the pick gate, checked against the `graphics.md` receipt, and `campaign.md` carries both of that gate's answers: the headline chosen out of how many ideas and the variant out of how many renders. An attachment this skill selected for itself is a finding, not a shortcut. And the coverage is checked the other way round: when the run has an image, every selected platform whose Media column reads `optional` or `required` declares it, with only the three exceptions from Phase 2 allowed and each one named in `campaign.md`. A picture attached to one post out of twelve is a finding, not a style choice.
13. Filename and frontmatter — every name parses back against the contract in Phase 6, and every file carries the fields that contract lists, `voice` among them. A `voice` naming a profile or a style guide is checked to be a path that exists, not a label; a post whose text was bent by its platform carries the `voice_deviation` line that says so.
