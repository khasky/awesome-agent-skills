...
**Which sentences may be anchors depends on the source type.** For a guide it is the table below: the proof, the split, the scope, the caveat, the setup blocks, the paths, the model, the gotcha. For an essay, a benchmark or a retrospective the anchor strings are only the sentences that carry a number with its condition, the quotations, and the fenced blocks the source itself carries, plus the thesis as one clause. Everything else the pack repeats there, the ladder of what the author reaches for, the verdict, the caveat as a judgment, the mechanism explanation, the prose around the proof, is a fact set: `anchors.md` records its elements (the rungs in order, the number and its condition, the thing not measured) and every file writes them in its own words. A run that maps an essay's ladder onto `split` because the table has that slot, or folds its verdict into `proof-full`, has registered its frames as anchors and will ship one closing to thirty files.

**Three things are frames however factual they feel**, at every level and for every source type, and putting any of them in `anchors.md` is what turns a pack into one post published thirty times: the **stance**, the **closing line**, and the **what-I-do-now ladder** the piece ends on. Their content is fixed and their wording is not: the ladder carries its rungs in the source's order with the escalation target unchanged, and each file says them in its own words. Measured on three packs of one source, the pack that had registered them shipped its ladder verbatim into 27 of 45 files and its verdict sentence into 26, and it was the pack the reader rejected on most of the long forms.
...
**For an essay the opener is the author's own situation**, or the source's own opening where the source already holds a post for that platform (`Platform sources`, `SKILL.md` Phase 0): what the author did, kept doing, or assumed, concrete and in the author's voice under first person, with the subject named. The three tests above are a guide's tests; an essay's opener passes when a reader could not mistake it for the opening of a post on another subject.

**One scene, one turn.** The first sentence may run long as long as it is one situation: the count of its words is not the test, and the openers a reader kept when three packs of one source were compared averaged 20 words with a third of them over 25. The test is that the judgment is not folded into the scene. The turn that sent the author looking is its own sentence after it (`<the two-word denial>`, `<the moment the author went and looked>`), at most 6 words, written for that post, and no turn sentence appears in more than 2 files, because a turn repeated across the pack is a tic. A quotation, a paper, a vendor document or the mechanism sentence opens at most 2 posts in the whole pack, and only where the source itself opens on it: every long form that opened on one of those in the measured packs lost to the version that opened on the author. A second-person imperative under first-person voice (`Open your <file> and count`) is not an opener either, since it hands the reader a chore before the author has said what they did.
...
- **Text diagrams** carry the mental model (`interface + tools = A / model provider = B / billing = C`), the routing split (`<cheap path> -> tasks / <stronger path> -> cases`), a before-and-after (`UI says A / traffic goes to B`), and a question reframed (`Which model? -> How do I route?`). Fenced `text`, ASCII, three to eight lines, one label per line. On surfaces that render fences they are the default representation for these four things; on surfaces that do not, the sentence stays and the block goes. A block the source itself carries travels everywhere fences render; a diagram the run drew lands in at most 3 files of the pack, since the same three lines in fifteen files is a template in monospace.
...
| split then link | The one-line split and the link. | hard caps of a guide |
| verdict line | One sentence of judgment with a consequence in it, written for that post from the fact set: what the number means, what the author does now, what the reader should stop expecting (`<what the number rules out>. <what is left>.` is the shape, not the text). Never the ladder, never a sentence from `anchors.md`, never shared by two files. | every post of an essay, benchmark or retrospective; the last line of every hard cap there, of every feed over 1,000 characters, and of every long form, where it may be the bold one-line lesson |

At least 4 shapes per pack, none on more than 40% of it, the author's recommendation sentence as the very last line on at most 6 posts of a full pack, no non-anchor closing sentence in more than 2 posts. Never a summary of the body, never `What do you think?`, never a maturity-curve line, never a slogan triplet or fragment rhythm (`Cheap by default, expensive on demand, local when ...`). A long form closes on one plain sentence of judgment (`That is a more useful way to think about <domain> than trying to pick one permanent "best <thing>."`), and a hard cap of a guide on `caveat-micro` or `split-micro` then the link. A hard cap of an essay closes on its verdict line then the link, and the ladder never closes any file: when three packs were compared, the hard caps that ended on the ladder lost 4 times in 5 and the ones that ended on a verdict won.
...
- no turn sentence (section 3) in 3 or more posts, and no `[A#]` author line in 3 or more posts or inside any anchor;
