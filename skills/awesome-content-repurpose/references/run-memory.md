...
One line per finished run: the date, the source (its URL, or `pasted` with the file name), a fingerprint of its text, the point the run took, the idea it used, the run folder, and whether the posts were published or left as files. The fingerprint is `node scripts/fingerprint.mjs repurpose/<slug>/source`, a hash of the extracted text with case and whitespace normalised, so the same text pasted twice matches even when the line breaks differ. The URL column catches the same page fetched again; neither catches the same article republished elsewhere with different surrounding text, and the check does not pretend to.
