...
4. **Platforms** (multi-select, ALWAYS asked, nothing pre-selected — the full list below is shown every run, and the answer is never guessed from what the sources look like): `facebook-wall` · `facebook-group` · `linkedin` · `reddit` · `lemmy` · `tumblr` · `mastodon` · `bluesky` · `x` · `threads` · `truthsocial` · `wonderful-dev` · `hackernoon` · `devto` · `hackernews` · `patreon` · `ko-fi` · `buymeacoffee` · `instagram` · `tiktok` · `pinterest` · `bastyon` · `vk` · `telegram` · `discord` · other (user names it; research it in Phase 3 like the rest). Facebook is two options because the surfaces differ (own wall/Page vs a moderated group); `lemmy` covers any Lemmy instance including `lemmy.world`, and `hackernews` is `news.ycombinator.com`. These slugs are the canonical vocabulary — filenames use them verbatim, and `awesome-post-publisher` parses against the same list. Adding a slug means adding it to `awesome-post-publisher` and to `references/platform-profiles.md` in the same change; a list that drifts leaves either posts the publisher cannot parse, or platforms the campaign can never write for. An empty selection is not a default to fall back on — re-ask.
...
Per-platform targets that posting requires — collect now, not at write time. **Every selected platform needs a target unless the account has exactly one global feed.** Page URL or personal-timeline URL for `facebook-wall` (ask which: a Page and a personal timeline are different surfaces with different tone expectations, and the publisher cannot infer it), group URL for `facebook-group`, subreddit for `reddit`, instance domain plus community for `lemmy` (`lemmy.world` is one instance among many, so both parts are needed), instance domain for `mastodon`, board for `pinterest`, wall or community for `vk`, channel or group for `telegram`, server plus channel for `discord`. `truthsocial` and `hackernews` post to a single global feed and need no target. Each answer goes into every affected post's frontmatter `target`. A post that reaches the publisher without the target its platform needs is a publication blocker, not a missing detail.
