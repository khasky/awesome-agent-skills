...
Turn raw product knowledge into a batch of dated, platform-fit posts a publisher (human or `awesome-post-publisher`) can ship on schedule. The pipeline: ingest sources → knowledge map → interview → live platform research → schedule and media → write to genre and voice → two-stage self-audit → files + manifest. The author's voice is not built here: question 9 points at a profile from `awesome-voice-profile`, and a campaign without one still runs on the voice chosen in the interview.
...
- `references/platforms.md` — **the canonical platform vocabulary**: the slug table with the target detail each platform needs, which platforms cannot post without media, and which genre file governs its register — plus structural notes per platform and the checklist of volatile limits to verify live. Deliberately carries no character-cap numbers; those rot, and Phase 3 fetches them fresh. `awesome-post-publisher` parses against this same table, so a platform is added here once, never restated in a SKILL.md.
...
4. **Platforms** (multi-select, ALWAYS asked, nothing pre-selected). Read `references/platforms.md` and show its full slug table every run — that file is the vocabulary, filenames use its slugs verbatim, and `awesome-post-publisher` parses against the same table. Plus one open option: any platform the user names that the table does not carry (research it in Phase 3 like the rest, and add its row). Facebook appears twice because the surfaces differ (own wall or Page versus a moderated group); `lemmy` covers any Lemmy instance including `lemmy.world`; `hackernews` is `news.ycombinator.com`. An empty selection is not a default to fall back on — re-ask.
...
Voice per the interview, held consistently across every post and platform. When question 9 produced a voice profile (from `awesome-voice-profile`, or a style guide from `awesome-style-mimic`), read it in full and apply it on top: its rhythm, lexicon, opener and closer habits, and its per-platform register where it has one. What wins what, when two of these disagree:
...
- Re-deriving a voice inside the campaign instead of reading the profile `awesome-voice-profile` produced, or treating a `confidence: low` profile as observed fact.
