...
7. **Media** — does the user have images or videos for the posts? Collect folder path(s) and any explicit per-post wishes ("the demo video goes with the launch post" — an explicit mapping always wins in Phase 4). The Media column of `references/platforms.md` names the platforms that cannot post without an attachment; when the library cannot cover them, offer the two honest options rather than shipping unpostable drafts: generate flat diagram graphics locally through `awesome-content-graphics` (HTML/CSS rendered offline, no image service, opt-in), or drop those platforms. A video-only platform can only be dropped: this skill does not produce video.
   ...
   **On a yes, collect what that skill builds the look from, in the same round**: a brand palette or brand assets, reference images or examples the user wants the set to sit next to, a render they approved on an earlier campaign, or a description in words. Any of them is a complete answer and so is none of them; the point is that the question is asked here, with the rest of the interview, rather than interrupting the run later.
...
**Generated graphics (opt-in, offline) — call the Skill tool with "awesome-content-graphics".** When the library cannot cover a media-required post, or a post's content is genuinely structural (steps, a comparison, one number, a quotable line from the sources), the image is built locally by that skill: self-contained HTML styled with CSS and screenshotted offline, no image-generation service, no API key, nothing about the campaign uploaded anywhere. Call it only on the Phase 2 yes, and hand it six things — the facts it may draw on with their conditions and provenance (from the knowledge map), the map's Unverified list as the boundary, `content-campaign/<slug>/media/` as the output folder, the look inputs the interview collected, the target ratio from the Phase 3 research, and the language the posts are written in, so the words on the graphic match them.

What that skill owns and this one does not reimplement: the set-size question (5, 25, 50, 100 or a number the user types), the spread of the set across its catalog, the headline gate before anything is drawn, the gallery, and the pick gate that decides which render ships. **This skill never chooses the graphic on the user's behalf**, and a campaign whose media came from the user's own library skips the whole path. What comes back is the chosen `.png`, its alt text and a `graphics.md` receipt; the manifest records the asset as generated rather than supplied, plus the headline chosen out of how many ideas and the variant chosen out of how many renders. The `.html` sources stay in `media/src/` so the user can edit and re-render without any skill.
