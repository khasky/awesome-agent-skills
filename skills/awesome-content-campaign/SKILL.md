...
7. Media — does the user have images or videos for the posts? Collect folder path(s) and any explicit per-post wishes ("the demo video goes with the launch post" — an explicit mapping always wins in Phase 4). The Media column of `references/platforms.md` names the platforms that cannot post without an attachment; when the library cannot cover them, offer the two honest options rather than shipping unpostable drafts: have the pictures drawn by `awesome-content-graphics`, which composes them itself with nothing fetched and nothing uploaded (opt-in), or drop those platforms. A video-only platform can only be dropped: this skill does not produce video.
   ...
   An image that exists is attached wherever the platform accepts one. Once the campaign has a picture for a post — from the library or generated — `awesome-content-image-adapter` is called with it and the folder that post set lives in, and writes the picture into each of those platforms' own frames, one `<slug>.png` beside each `<slug>.md`, before any attachment is declared. Every platform that post targets whose Media column reads `optional` as well as `required` then names the file sitting next to it, and the article platforms take it as the cover image. Media is not a per-platform privilege reserved for the ones that would otherwise be unpostable. The exceptions are the same three: platforms that support no media at all, video requirements a still cannot meet, and a post where the user or the copy says the image does not belong, which the manifest records.
