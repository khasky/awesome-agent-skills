...
4. **Platforms** (multi-select, ALL checked by default): `facebook-wall` · `facebook-group` · `linkedin` · `reddit` · `tumblr` · `mastodon` · `bluesky` · `x` · `threads` · `wonderful-dev` · `hackernoon` · `devto` · `patreon` · `ko-fi` · `buymeacoffee` · `instagram` · `tiktok` · `bastyon` · `pinterest` · `vk` · `telegram` · other (user names it; research it in Phase 3 like the rest). These slugs are the canonical vocabulary — filenames use them verbatim, and `awesome-post-publisher` parses against the same list. Adding a slug means adding it to `awesome-post-publisher` and to `references/platform-profiles.md` in the same change; a list that drifts leaves either posts the publisher cannot parse, or platforms the campaign can never write for.
