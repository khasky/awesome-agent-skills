...
Images are not made here. `awesome-content-graphics` owns that stage whole — the look inputs, the variant set, the headline question and the pick gate — and Phase 4 calls the Skill tool with "awesome-content-graphics" instead of carrying its own copy of the rules. This skill supplies the facts and the boundary, and records the two answers the user gave it.
...
Generated graphics (opt-in, offline) — call the Skill tool with "awesome-content-graphics", the post's title travelling with the facts as the first line of its headline pool. When the library cannot cover a media-required post, or a post's content is genuinely structural (steps, a comparison, one number, a quotable line from the sources), the image is built locally by that skill: self-contained HTML styled with CSS and screenshotted offline, no image-generation service, no API key, nothing about the campaign uploaded anywhere. Call it only on the Phase 2 yes, and hand it six things — the facts it may draw on with their conditions and provenance (from the knowledge map), the map's Unverified list as the boundary, `content-campaign/<slug>/media/` as the output folder, the look inputs the interview collected, the target ratio from the Phase 3 research, and the language the posts are written in, so the words on the graphic match them.
...
Validation is part of this phase, not a hope, and once the posts exist `node scripts/check-campaign.mjs --campaign content-campaign/<slug>` settles the mechanical half of it: exit 0 or a `reason=` per finding. After generating names, parse every one back against `^(\d{4}-\d{2}-\d{2})_(\d{2}-\d{2})_([A-Za-z][A-Za-z0-9-]*)_([a-z0-9][a-z0-9-]*)_(<slug-list>)\.(md|txt|csv|html|pdf)$` and check no platform has two posts in one slot. A name that fails the round-trip is fixed before writing content into it.
...
- No AI-vocabulary tier-1 words (delve, seamless, robust, tapestry, testament, boasts, leverage-as-verb, deliberately-as-intent-stamp), no "it's not X, it's Y" contrast frames, no forced rule-of-three, no negative-parallelism countdowns ("No X. No Y. Just Z.").
