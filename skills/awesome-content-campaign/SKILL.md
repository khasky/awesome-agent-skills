...
Images are not made here. `awesome-content-graphics` owns that stage whole — the look inputs, the variant set, the headline gate and the pick gate — and Phase 4 calls it instead of carrying its own copy of the rules. This skill supplies the facts and the boundary, and records the two answers the user gave it.
