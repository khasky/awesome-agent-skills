...
Whatever the state, the level-(c) platforms are not quietly dropped. Present the choice and wait: **fix the bridge now** (the token procedure, then continue) · **skip these platforms**, recorded as skipped in the profile and the report · **stop here** and resume when the browser is ready. Announcing a fallback and proceeding is not one of the three — a run that collects two public feeds and calls the profile done has answered a question the user was never asked.
