...
1. **The user's own posts on their platforms** — profile URLs handed to this skill are this source, not source 4: a link to the user's own account on a platform means "learn from what I posted there", and routing it to a site crawl instead is a misread.
   ...
   **Three levels of access, and the cheapest one that works wins.** (a) A plain fetch, when the profile is server-rendered and public — no browser, no bridge. (b) A browser, any browser, when the page is a JS app that returns an empty shell to a fetch — most modern feeds are. (c) The user's *own* logged-in browser through the bridge, only for what a logged-out visitor cannot see: members-only posts, a platform that hides the archive behind a login wall, an account set to followers-only. Say per platform which level was used; a profile read at level (a) or (b) is public evidence and needs no session at all.
