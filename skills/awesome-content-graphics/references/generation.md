...
The agent composes every picture itself. There is no catalog to pull from and no service to ask, so the variety has to be decided rather than hoped for: a hundred pictures made without a plan are one picture made a hundred times, whoever is drawing.
...
- **The scene without the subject**, the place it belongs to, recognizable by its absence.
...
Each picture is written out as markup the agent produces on the spot — vector shapes, type, fields of color — and rasterized to a PNG at the ratio the run agreed. Four things decide it and nothing else matters:
...
**Vary the axis that shows, not the one that does not.** At the size a post is read, what separates two pictures is the kind of picture, where the subject sits, the light and the palette. A restyle, a color swapped for its neighbor, a border added — those produce two files and one picture. Spend the count on the axes a viewer can see.

**What this medium can and cannot do.** Composed markup gives flat and graphic work: shape, type, color, diagram, pattern, silhouette, texture built from primitives. It does not give photography, and a run that promises a photographic set through this route is promising something it cannot draw. Where the material wants a photograph, say so and let the user supply one — `awesome-content-image-adapter` will fit whatever they bring to every platform.
...
Reusing the first round's ideas with new colors is the first round again. So is keeping the compositions and changing the type.
...
- **Every file opens** and is an image at the ratio that was asked for. A file that failed to rasterize is a finding, not a detail.
...
- **The claims.** No number, logo, named product or recognizable person that the material does not support.
- **The subject survived.** Cover the caption and ask whether the subject is still recognizable. A set that needs its caption to make sense is a set of decorations.
