...
| Grain | A tiled noise overlay at 3 to 8 percent | Turns a flat print into a printed one. Rasterise it once at build time into a small tiled PNG and embed that, never as a fetched texture and never as a live `feTurbulence` filter on the canvas: a filter under a blend mode stalls the capture outright. Never on a photo canvas — the photograph already carries the texture, and grain blended over a full-bleed image is the reliable way to wedge the renderer |
