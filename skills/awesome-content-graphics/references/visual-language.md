...
- Size the canvas to the platform, from the caller's verified research rather than memory — the ratios in play are the vertical feed image, the square, and the link-preview landscape. Set the body to exact pixel dimensions and **capture at 1×**. The canvas is already authored at the delivery size — 1080×1350, 1080×1080 — so a 2× capture buys nothing a feed will ever show, and it costs the run dearly: the clip scale is what stalls the compositor (below), and one set spent the best part of an hour on 45-second capture timeouts and browser relaunches before dropping to 1× rendered all 100 pages clean on the first attempt. Upscale only for a platform whose verified research asks for a larger file, and treat that as the exception it is.
...
One browser for the whole set: find a Chromium-family binary the way the machine exposes it (an explicit path in the environment first, then an automation cache, then an installed browser), drive it over its own debugging protocol, set the viewport once, load each page, wait for fonts, run the check and capture only on a pass. Measure and capture at device pixel ratio 1.

Which layers stall, precisely, because the run that hit them lost an hour to it. A capture above 1× hangs on pages carrying a **live filter under a blend mode** and on **full-bleed photographs**; the same pages capture instantly at 1×. Two of them are avoidable outright and both fixes belong in the build rather than the renderer: rasterise the grain once into a small tiled PNG at build time instead of leaving an `feTurbulence` filter live under `mix-blend-mode` on every canvas that carries it, and put no grain at all on a photo canvas — the photograph already supplies the texture grain exists to add, and grain blended over a full-bleed image is the single most reliable way to wedge the compositor. A capture that does stall never recovers on its own: give it a timeout, and on expiry replace the whole browser rather than the page, because the renderer stays wedged and every later page inherits it.
...
## Running the pass without losing an hour

**Prove the plan on a probe before building the hundred, and build the probe from layouts rather than kinds.** One row for every layout the plan uses, through the gate first. Almost every defect that stops a set is structural and shows up in the first example of its *layout*, not of its kind: a column that cannot fill its height, a value that overruns its own box, a figure whose short bar leaves the row empty. A probe of 7 rows covering the 5 kinds passed clean and let 8 rounds of geometry defects through to the hundred-page pass; a probe of 27, one per layout, measured 82 seconds.

**A gate failure re-renders the rows that failed, never the set.** The renderer takes a row list for exactly this reason. One run ignored it and re-ran all one hundred pages after every fix — thirteen full passes for defects that touched one to twelve rows each. A full pass measures about two and a half minutes on this catalog, so the passes themselves were half an hour of the hour and three quarters; the rest went on the diagnosis rounds around them, which is what the probe and the gate's coordinates remove. Either way it is waste: the passing PNGs on disk are still correct, and the sweep at the end is what re-checks the set as a whole. Re-render everything only when a change touches every page — the palette pool, the canvas size, the shared fit.

Two mechanical traps, both of which cost a working pass. Do not pipe a long render through `tail` or `head`: the pipe buffers, the run looks hung, and a healthy pass gets killed and restarted. And do not background it with a shell `&` — it dies with its shell and leaves the browser processes orphaned, which then compete with the next attempt. Run it in the foreground, or through whatever the runtime offers for a tracked background task, and on Windows kill the browser by process tree rather than by parent PID, because the shell spawns children that outlive it. A sweep killed this way dies after the contact sheets and before the gallery, and the gallery left on disk looks current while indexing a previous generation of renders.

The pipeline that does all of this ships with the skill under `tools/`; `references/render-pipeline.md` carries its interface and the traps it already answers.
...
Two notes on running it. Read the rects after fonts have settled and after the page's own fit has run (`window.__fitted`) — `await document.fonts.ready` before the evaluate, or a wide headline measures short and passes a check it should fail.

`document.fonts.ready` is not enough inside the page's own fit, and this one is worth stating plainly because it wastes whole render passes. That promise resolves when the font loads *currently pending* have finished — and a headline block the build emits empty has requested nothing, so the promise resolves immediately with the fallback face active. The fit then measures system-ui metrics, sizes the type for a width the page will never have, and the real face swaps in afterwards: a narrow serif came out at 168px where it should have been 150 across two lines, tripping the fill gate on canvas after canvas and sending the run back for another full pass. Load the face explicitly before fitting — `await Promise.all(weights.map(w => document.fonts.load(w + ' 100px "' + face + '"')))`, then `document.fonts.ready` — and have the page publish the face name it is waiting for so the script does not have to guess it. And `getBoundingClientRect` covers transforms and layout but not a `filter: blur()` halo or a large `box-shadow`; where a blurred blob is doing background work, leave it untagged, since a background is allowed to cross the edge and only marks are being measured.
