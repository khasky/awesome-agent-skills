...
**Plan the whole set as signatures before writing a line of markup.** Each variant is a row — palette, background recipe, kind, subject, face and type effect, anchor — and the rule from `references/style-catalog.md` is that no two rows are identical and none match on more than three of the six. Collisions are cheap to fix in a table and expensive to fix in a rendered set, and the table is what makes 100 variants honestly distinct rather than 100 files.
...
**Fetch the faces and the icon set before the first variant is planned.** They are part of the system, not an afterthought: a run that starts drawing before the assets are on disk ends up reaching for emoji every time, which is how a set of a hundred comes back with two devices in it.
