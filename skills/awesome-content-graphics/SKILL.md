...
description: "Produces post graphics offline: a user-chosen set size (10, 50, 100 or any number) of self-contained HTML/CSS variants rendered locally to PNG, across glyph canvases, display-scale number lockups, data figures, comparison cards and commercially usable stock photographs, built from the supplied facts and the user's own look inputs (brand palette, reference images). The source may be a URL, file or text in any language; one screen settles the language on the canvas, the set size and the headlines, and a pick gate settles which render ships when a post skill calls it. Called on its own it hands over the whole set. No image service, no API key, nothing uploaded. Use when asked to make an image or graphic for a post or campaign, 'сделай картинку для поста', or whenever awesome-content-campaign or awesome-content-repurpose reach a platform that cannot post without media. Do not use for a photograph or an illustration made to order, video, or writing the posts themselves."
license: MIT
metadata:
  author: Khasky
...
| Look inputs | The user's answer to the Phase 1 look question: Random, a reference, or custom info in their words | Random: the run builds the system from the source's own context, a validated scheme per variant, and says that is what happened |
...
A graphic that ignores what the user handed it is a graphic they will reject, whatever its geometry. So this phase runs before the facts are ranked and before a headline is written, and it settles the visual system the whole set is built in. It is one question with exactly three options, and the wording is fixed. It is asked in the same round as the language and the set size (Phase 3) and the headline mode (Phase 4): four independent answers, one screen, never four rounds. The look and the set size are two of those answers, never one option that bundles them.

| # | Option | What it means |
| --- | --- | --- |
| 1 | Random (based on the post topic, tone, purpose) | The run builds the whole system itself from the source's own context — palettes, grounds, faces, icons, photo queries, motifs — a fresh validated scheme per variant. The default |
| 2 | Reference (URL, text description, or another image) | Reference mode. The next step asks for the reference itself: a site or page URL, a description in words, or a path to an image or a file, and everything is extracted from it |
| 3 | Custom info | Free text: a brand's hexes, a palette, anything the user wants to say. Where the runtime already offers its own free-text entry (`Type something`), that entry is this row and the skill adds no third row |

Random is built from the post, never rolled. The source handed to the run (the text, the URL, the facts a caller passed) decides what the set looks like before any lever is rotated: what the thing is picks the photo queries and the icon vocabulary (a storage engine gets drives and memory modules, a design tool gets paper and pencils); who it is for picks the register the palettes are drawn from (a developer tool sits on deep and technical grounds, a consumer app on light and warm ones); the headline pool decides which rows carry a lockup, a figure or a list. The levers then move underneath that reading so the set reads as many, and the receipt says what the reading was. A user who wants to steer it types the steer into option 3.

The question never carries anything else, and these are hard rules, not preferences:

- Never offer a mood as an option, and never dress option 3 up as one. "Dark, cinematic, hot accent", "clean and minimal", "editorial", `Dark technical, monospace`, `Light and plain`, `Describe it: dark, technical` — none of them. Everything in the templates is used automatically, under the hood, and offering a mood asks the user to do the run's job while pretending it is a choice. A user who wants a mood types it into option 3.
- Never offer an earlier run's look as an option. `Same look as last run`, `Same family as the last set`, `An approved render from an earlier run` — none of them, whether the earlier run was this skill's or a caller's. The run has no such render, a caller's defaults file carries no look, and the option reads as the run remembering work it never saw. A user who wants an earlier render as the look points at its file under option 2.
...
- Never propose a reference the user did not name — least of all the source article's own site. The source supplies facts; it has no claim on the look, and `build it from fal's own site` is the run inventing a brand relationship nobody asked for. A reference exists only when the user names it under option 2.
- Never offer to pin a kind, an archetype or a layout. The kinds and their quotas are the run's business.
- Three options, no fourth, and the labels are the three rows above verbatim: the first is always `Random (based on the post topic, tone, purpose)`, the second is always `Reference (URL, text description, or another image)`, the third is the free-text entry. The runtime's chat entry is not an option this skill adds to.
- The same three rows wherever the question is asked. A calling skill that collects the look in its own interview asks it with these labels and hands the answer over; it never writes its own version of the question.

Option 2 collects the reference in the next step, not in the question itself. The user picks, and the run then asks for the URL, the description or the path in one short follow-up — never a question that demands the URL be typed into an option field.
...
Then every palette is proved, not eyeballed, and for a no-reference set that proof is already done. `references/palettes.md` holds 43 schemes that passed the colour rule when the pool was proved: accent and muted neutral at 3:1 or better against their own ground, foreground at 4.5:1, accent and muted still apart under protan and tritan simulation. The set draws its scheme-per-variant from that pool, spread across its hues and its dark and light halves, and validates nothing per run. Only a colour that did not come from the pool is checked at run time — a brand hex, a reference site's palette, a scheme the user typed — by measuring the four colours against the rule — accent and muted at 3:1 on the ground, foreground at 4.5:1, accent and muted still apart under protan and tritan simulation — and naming which of the four fails. A brand colour that fails is reported with what it fails, and the user decides between their colour and the check; it is never silently swapped for one that passes.
...
Two disciplines around that pass, and they are the difference between a set that takes minutes and one that takes an hour.

**Prove the pipeline on a handful before building the hundred.** Render six or so rows chosen to cover every kind the plan uses — a glyph, a lockup, a figure, a list, a photo — and take those through the gate first. Almost every defect that stops a set is structural and shows up in the first example of its kind: a fit that measures the wrong face, a value that overruns its canvas, a layer that stalls the capture. Finding one of those on six pages costs a minute; finding it on the hundredth page costs the pass.

**A gate failure re-renders the rows that failed, never the set.** The renderer takes a row list for exactly this reason. One run ignored it and re-ran all one hundred pages after every fix — eleven full passes for defects that touched one to twelve rows each, at six to ten minutes a pass. That single habit was the bulk of the hour the set took, and it is pure waste: the passing PNGs on disk are still correct, and the sweep at the end is what re-checks the set as a whole. Re-render everything only when a change touches every page — the palette pool, the canvas size, the shared fit.

Two mechanical traps, both of which cost a working pass. Do not pipe a long render through `tail` or `head`: the pipe buffers, the run looks hung, and a healthy pass gets killed and restarted. And do not background it with a shell `&` — it dies with its shell and leaves the browser processes orphaned, which then compete with the next attempt. Run it in the foreground, or through whatever the runtime offers for a tracked background task, and on Windows kill the browser by process tree rather than by parent PID, because the shell spawns children that outlive it.
...
The report also quotes the render pass's own tally — pages passed out of pages built, the findings, the renderer that was used — and where the per-page measurements were written, because a set reported as gated without those numbers is the claim this phase exists to replace; and it names the asset cache used and whether a style pack was reused from it.

No renderer available at all (no Chromium-family browser on the machine and none named in the environment) → say so and hand back nothing rather than promising an image. The caller ships the posts text-only and records why.
...
- Offering a mood, a pinned kind, or a reference the user never named in the look-input question — including the source article's own site. Three options: Random, a reference, custom info.
- A look question whose labels are not the three rows: `Dark technical, monospace, 50 renders` or `Same look as last run` in place of Random, `Describe it: dark, technical` in place of custom info, the look and the set size bundled into one option, or a caller asking the question in its own words. The first option is `Random (based on the post topic, tone, purpose)`, and Random reads the post to build the system rather than rolling dice.
