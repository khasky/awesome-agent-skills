...
A second exception, and it is about cost. `awesome-content-graphics` renders a set by generating a browser-driving toolchain — asset loader, page scaffolding, layout table, geometry gate, renderer, sweep — and generating it again on every run is the largest single cost the skill has: one measured run spent about 45 minutes writing it and another hour and three quarters rediscovering defects the previous run had already solved, among them a height model that ignores the real line box, a fit that measures a fallback face because the effect's face was never loaded, and a value that paints outside its own box while its rect stays inside it. So that toolchain ships, under `skills/awesome-content-graphics/tools/`, and `references/render-pipeline.md` carries its interface, its contract and the traps it already answers. The exception is exactly this narrow: code the agent runs on its own machine, against pages it generated itself, writing only into its own temporary folder. It never covers a checker aimed at the reader's own project, nothing there is copied into a user's repository, and no other skill gains a `tools/` folder by pointing at this one. The `skills ship Markdown only` gate in `scripts/lint.py` carries the same allowlist, so the exception is enforced rather than remembered.
